...
namespace SerialTerminalApplication
{
    public partial class Form1 : Form
    {
        ...
        string baudRate, portName, logPath, readPath, writeFilePath;
        int w1, w2, w3;
        ...
        private List<byte> TestSendType(string s)
        {
            ...
            if (s.StartsWith("" + '"') && s.EndsWith("" + '"')) foreach (byte b in Encoding.ASCII.GetBytes(s.Substring(1,s.Length-2))) { bA.Add(b); }
            else
            {
                ...
                {
                    bA.Add(Convert.ToByte(Convert.ToInt16(s)));
                }
            }           
            return bA;
        }
        ...
        private void LoadWriteFile()
        {
            try
            {
                System.IO.StreamReader writeFile = new System.IO.StreamReader(writeFilePath);
                String[] fileList = writeFile.ReadToEnd().Split(';');
                UpdateStatus("Sending data via. file.");
                foreach (String line in fileList)
                {
                    ...
                    try
                    {
                        ...
                        {
                            string sn = s.Replace("\r\n", "").Trim();
                            if (sn.Remove(0, sn.Length - 2) == "ms") Thread.Sleep(Convert.ToInt32(sn.Remove(sn.Length-2,2)));
                            else if(sn.StartsWith("0x")) bA[i++] = Convert.ToByte(Convert.ToInt32(sn, 16));
                        }
                        WriteByteArray(bA);
                    }
                    catch (Exception)
        ...
        private void ReadData()
        {
            if (InvokeRequired)
            {
                Invoke(new MethodInvoker(ReadData));
            }
            else
            {
                ...
                switch (readDataFormatCBox.Text)
                {
                    ...
                    case "DEC":
                        for (int i = 0; i < BytesToRead; i++) if (!holdCBox.Checked) UpdateReadBox(Convert.ToInt32(serialPort.ReadByte()).ToString() + ", ");
                        break;
                }
            }
        ...
        {
            if (!(browseSendTBox.Text == ""))
            {
                writeFilePath = browseSendTBox.Text;
                Thread LoadFile = new Thread(new ThreadStart(LoadWriteFile));
                LoadFile.Start();
            }
